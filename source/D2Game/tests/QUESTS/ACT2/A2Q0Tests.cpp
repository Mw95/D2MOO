...
TEST_SUITE("A2Q0Tests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F350" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_InitQuestData, dll_base + 0x0006F350);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F3F0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_Callback11_ScrollMessage, dll_base + 0x0006F3F0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F460" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_Callback00_NpcActivate, dll_base + 0x0006F460);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F510" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_ActiveFilterCallback, dll_base + 0x0006F510);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F540" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_Callback10_PlayerLeavesGame, dll_base + 0x0006F540);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC9F560" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q0_Callback13_PlayerStartedGame, dll_base + 0x0006F560);
