	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA0240" * doctest::skip(""))
	{
		...
		SUBCASE("")
		{
			...
			const auto setup_data = []() {
				...
				return std::tuple{ pQuestData, pUnit, pData };
			};
			
			// Input data
			auto [moo_pQuestData, moo_pUnit, moo_pData] = setup_data();
			auto [original_pQuestData, original_pUnit, original_pData] = setup_data();

			// Call both implementations
			const auto moo_result = sut(&moo_pQuestData, &moo_pUnit, &moo_pData);
			const auto original_result = original(&original_pQuestData, &original_pUnit, &original_pData);
