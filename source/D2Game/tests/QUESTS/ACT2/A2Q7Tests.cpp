...
TEST_SUITE("A2Q7Tests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6420" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q7_InitQuestData, dll_base + 0x00076420);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA64B0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q7_Callback11_ScrollMessage, dll_base + 0x000764B0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6510" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q7_Callback02_NpcDeactivate, dll_base + 0x00076510);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6540" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q7_Callback00_NpcActivate, dll_base + 0x00076540);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6660" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(OBJECTS_InitFunction05_Door, dll_base + 0x00076660);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6680" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT2Q7_ActiveFilterCallback, dll_base + 0x00076680);
