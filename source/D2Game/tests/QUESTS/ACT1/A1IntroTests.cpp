...
TEST_SUITE("A1IntroTests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC975E0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Intro_InitQuestData, dll_base + 0x000675E0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC97640" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Intro_Callback11_ScrollMessage, dll_base + 0x00067640);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC976C0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Intro_Callback00_NpcActivate, dll_base + 0x000676C0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC97770" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Intro_ActiveFilterCallback, dll_base + 0x00067770);
