...
TEST_SUITE("A1Q0Tests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC977D0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Q0_InitQuestData, dll_base + 0x000677D0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC97830" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Q0_Callback11_ScrollMessage, dll_base + 0x00067830);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC97870" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Q0_Callback00_NpcActivate, dll_base + 0x00067870);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FC978F0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT1Q0_ActiveFilterCallback, dll_base + 0x000678F0);
