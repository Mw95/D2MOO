...
TEST_SUITE("A5IntroTests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB0FD0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT5Intro_InitQuestData, dll_base + 0x00080FD0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB1030" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT5Intro_Callback11_ScrollMessage, dll_base + 0x00081030);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB1100" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT5Intro_Callback00_NpcActivate, dll_base + 0x00081100);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB11D0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT5Intro_ActiveFilterCallback, dll_base + 0x000811D0);
