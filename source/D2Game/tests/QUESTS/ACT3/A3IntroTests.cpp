...
TEST_SUITE("A3IntroTests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA68D0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT3Intro_InitQuestData, dll_base + 0x000768D0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6930" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT3Intro_Callback11_ScrollMessage, dll_base + 0x00076930);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6A00" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT3Intro_Callback00_NpcActivate, dll_base + 0x00076A00);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6B00" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT3Intro_StatusFilterCallback, dll_base + 0x00076B00);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCA6B10" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT3Intro_ActiveFilterCallback, dll_base + 0x00076B10);
