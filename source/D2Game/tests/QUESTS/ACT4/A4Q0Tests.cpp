...
TEST_SUITE("A4Q0Tests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCAD6F0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q0_InitQuestData, dll_base + 0x0007D6F0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCAD750" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q0_Callback11_ScrollMessage, dll_base + 0x0007D750);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCAD790" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q0_Callback00_NpcActivate, dll_base + 0x0007D790);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCAD800" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q0_ActiveFilterCallback, dll_base + 0x0007D800);
