...
TEST_SUITE("A4Q4Tests")
{
	const auto working_directory = std::filesystem::current_path();
	const auto dll_base = reinterpret_cast<uintptr_t>(LoadLibraryA((working_directory / "D2Game.dll").string().c_str()));

	
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB0E50" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q4_InitQuestData, dll_base + 0x00080E50);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB0EB0" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q4_Callback00_NpcActivate, dll_base + 0x00080EB0);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCB0F50" * doctest::skip(""))
	{
		// Set up function pointers
		const auto [sut, original] = make_function_pair(ACT4Q4_ActiveFilterCallback, dll_base + 0x00080F50);
