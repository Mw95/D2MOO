...
TEST_SUITE("A4Q1Tests")
{
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCADD00" * doctest::skip(""))
	{
		...
		SUBCASE("")
		{
			...
			// Call both implementations
			const auto moo_result = sut(&moo_pGame, &moo_pPlayer, moo_pData);
			const auto original_result = original(&original_pGame, &original_pPlayer, original_pData);
	...
	TEST_CASE_FIXTURE(NoopFixture, "D2Game.0x6FCAE400" * doctest::skip(""))
	{
		...
		SUBCASE("")
		{
			...
			// Call both implementations
			const auto moo_result = sut(&moo_pGame, &moo_pTargetUnit);
			const auto original_result = original(&original_pGame, &original_pTargetUnit);
